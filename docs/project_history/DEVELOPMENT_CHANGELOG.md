...
## [v6.5] - 2026-02-04
### Bug Fix: X-bar/S Chart Blank Page (Stability Improvement)

#### Task Overview
Resolved a critical issue where selecting "X-bar/S Chart" resulted in a blank page for datasets with more than 10 cavities.

#### 1. Statistical Constant Extension (`spc_logic.js`)
- **Issue**: Constants ($A_3, B_3, B_4$) were only defined for $n \le 10$, returning `null` for larger subgroups.
- **Solution**:
    - Extended $A_3, B_3, B_4, c_4$ constants up to $n=25$.
    - Implemented **Large-Subgroup Approximation Formulas** for $n > 25$:
        - $c_4 \approx 1 - \frac{1}{4n}$
        - $A_3 = \frac{3}{c_4 \sqrt{n}}$
        - $B_3 = \max(0, 1 - \frac{3}{c_4 \sqrt{2(n-1)}})$
        - $B_4 = 1 + \frac{3}{c_4 \sqrt{2(n-1)}}$

#### 2. Robust UI Rendering (`App.jsx`)
- **Safety Checks**: Implemented Optional Chaining (`?.`) and default fallbacks (`|| 0`) when accessing `xbar_s_chart` properties.
- **Error Prevention**: Added `isNaN` and zero-check for standard deviation calculations in the Zone shapes logic to prevent Plotly crashes.

#### 3. S-Chart Visualization Fix (`App.jsx`)
- **Data Binding**: Correctly mapped `s_values` and S-chart control limits (`ucl_s`, `lcl_s`, `s_bar`) to the second plot area.
- **Dynamic Legends**: Updated titles and hover templates to distinguish between R, MR, and S charts.

#### Files Modified
- `frontend/src/utils/spc_logic.js`: Extended constants and added approximation logic.
- `frontend/src/App.jsx`: Added safety checks and fixed S-chart rendering.
